...
export const AboutSection: React.FC<AboutSectionProps> = ({ content }) => {
  const story = content?.story || {
    ...
    content: "Setiap produk kami dikerjakan dengan penuh dedikasi. Kami percaya bahwa barang buatan tangan memiliki energi unik yang membawa keceriaan dan kenyamanan bagi si pemakai.",
    stats: "1+ Tahun",
    statsLabel: "Menemani momen berharga anak-anak dengan rajutan tangan berkualitas."
  };

  const features = content?.features || [
    {
      title: "Safe & Pure",
      description: "Benang katun premium yang lembut & aman untuk kulit bayi.",
      icon: "shield"
    },
    {
      title: "High Detail",
      description: "Kerapian rajutan tingkat tinggi dengan kontrol kualitas yang ketat.",
      icon: "sparkles"
    }
  ];

  const images = content?.images || ["/aboutImages/aboutImg1.png", "/aboutImages/aboutImg2.png"];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="relative">
            ...
            <div className="absolute md:-bottom-20 -right-4 -top-20 md:top-75 bg-white opacity-85 p-8 rounded-3xl shadow-2xl max-w-[240px] border-none ring-1 ring-brand-primary/5">
              <p className="font-display text-4xl font-black italic mb-2 text-brand-primary">{story.stats}</p>
            ...
            <div className="space-y-8 text-brand-primary/60 text-lg leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-4">
                {features.map((feature, index) => (
                  <div key={index} className="flex flex-col gap-4 p-6 rounded-3xl bg-brand-muted/50 border border-brand-primary/5">
                    <div className={`w-10 h-10 ${index === 0 ? "bg-brand-secondary" : "bg-brand-primary"} rounded-xl flex items-center justify-center text-brand-primary`}>
                       {index === 0 ? (
                    ...
                    <h4 className="font-display text-brand-primary text-xl font-bold">{feature.title}</h4>
