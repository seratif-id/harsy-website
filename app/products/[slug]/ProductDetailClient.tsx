          ...
          <div className="space-y-6 lg:max-w-md mx-auto w-full h-full flex flex-col justify-center">
            <div className="aspect-[3/4] lg:aspect-auto lg:flex-1 bg-brand-muted rounded-[2rem] overflow-hidden relative shadow-xl shadow-brand-primary/5 border-[6px] border-white group w-full">
               ...
               <div className="absolute top-6 right-6 z-10 flex flex-col gap-1 items-end">
                 {(() => {
                   const isNew = [...allProducts]
                     .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
                     .slice(0, 4)
                     .some(p => p.id === product.id);
                   
                   const featuredBadges = getProductBadges(product, allProducts);
                    ...
                    return (
                      <div key={review.id} className="bg-white p-6 rounded-[2rem] shadow-lg shadow-brand-primary/5 border border-brand-primary/5 group hover:-translate-y-2 transition-all duration-500">
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-12 h-12 bg-brand-muted rounded-xl flex items-center justify-center shadow-inner overflow-hidden relative">
                            {user?.avatar ? (
                                <Image src={user.avatar} alt={user.name} fill className="object-cover" />
                            ) : (
                                <div className="text-brand-primary font-bold text-lg">{user?.name ? user.name.charAt(0) : "U"}</div>
                            )}
                          </div>
                          <div>
